...
c_bbc_display::clock( void )
{
    if (clocks_to_call>0) {
        ...
        if (reset_cycle==1) {
            add_pending_csr_write_request(bbc_csr_select_clocks, 0, (1<<16) |(2<<8) | (3<<0) );// clock speedup
            add_pending_csr_write_request(bbc_csr_select_clocks, 0, (0<<16) |(2<<8) | (3<<0) );// clock speedup
